...
class UnifiedPipeline:
    ...
    def process(self, query: str, response: str = None) -> RoutingDecision:
        ...
        # Step 2: Determine model selection with TWO-TIER FALLBACK
        # Thresholds: global=0.80, specialist LOW band=0.45-0.79, simple_qa LOW band=0.60-0.79
        preferred_model = self.classifier.get_preferred_model(classification.label)
        
        if classification.confidence < self.confidence_threshold:
            # Below global threshold - check tier (vague ALWAYS goes safe)
            if classification.is_vague:
                # Vague query - safe route (never use LOW-band OK)
                ...
                # Simple QA in LOW band (0.60-0.79) → route to intended cheap model
                ...
                # Specialist in LOW band (0.45-0.79) → route to preferred specialist model
                selected_model = preferred_model
                fallback_triggered = True
                ...
                selected_model = self.safe_fallback_model
                fallback_triggered = True
                fallback_reason = f"VERY_LOW_CONF_FALLBACK: conf={classification.confidence:.2f} < 0.45"
            else:
                # Other cases (shouldn't happen given above) → safe fallback
                selected_model = self.safe_fallback_model
                fallback_triggered = True
